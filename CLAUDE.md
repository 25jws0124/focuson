...
├─ js/
│  ├─ app.js             오케스트레이션만. 엔진 배선 + 버튼 + 카메라
│  ├─ focus-engine.js    ★ 진짜 엔진. 설정은 상단 DEFAULT_CONFIG 한 곳
│  ├─ mock-engine.js     같은 계약의 가짜 엔진 (?mock)
│  ├─ engine-contract.js 엔진과 화면이 공유하는 값 (집중 임계선)
│  └─ ui/
│     ├─ dom.js          화면 요소 참조. getElementById 는 여기서만
│     ├─ theme.js        색(:root 에서 읽음) + 상태 한글명
│     ├─ display.js      화면 수치·문구. HTML에 숫자를 안 적고 여기서 주입
│     ├─ charts.js       캔버스 그리기 3종
│     ├─ report.js       학습 리포트 렌더
│     └─ battle.js       배틀룸 (백엔드 있을 때만 노출)
...
정적 서빙 루트 노출 · 학부모 이메일 로그 · 화면 문구 부정확 · 하드코딩 이중 관리 6건 ·
`hidden` 무력화 · socket.io 무한 재시도 · 백엔드 없을 때 앱 중단 · 인증서 깨짐 시 서버 부팅 실패 ·
소켓 CORS `origin:'*'` · hex 색상 중복 · `app.js` 단일 파일 비대화

### 알고 있는 한계 (결함이 아니라 미구현 — 심사에서 물으면 그대로 답한다)

- 배틀룸 방 정보가 메모리 `Map` 이라 서버를 재시작하면 진행 중인 방이 사라진다.
- 학부모 리포트는 발송 로직만 있고 실제 이메일은 나가지 않는다. 이메일 서비스만 붙이면 된다.
- '막힌 지점'은 시각만 표시하고 영상 미리보기는 없다 (영상을 저장하지 않기 때문이다 — 원칙 ①의 결과).
- 정적 배포본에는 배틀룸·학부모 발송이 없다. 두 기능은 로컬에서 `server/` 를 띄웠을 때만 동작한다.
